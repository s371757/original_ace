...
for j in range(len(act_concept), len(X)):
    ...
    logistic_model_leave_out = LogisticRegression(max_iter=1000)
...
# Step 3: Compute LOO scores and plot orthogonal lines to LOO boundaries
for j in range(len(act_concept), len(X)):
    loo_score = newton_estimate_loo(logistic_model_full, X_pca_2, y, j)

    # Compute the slope of the LOO decision boundary
    if loo_score[1] != 0:
    ...
    if loo_slope != np.inf:
        orthogonal_slope = -1 / loo_slope
    else:
        orthogonal_slope = np.inf
    
    # Create an orthogonal line based on the LOO slope
    x_vals = np.linspace(x_min, x_max, 100)
    if orthogonal_slope != np.inf:
        y_vals = orthogonal_slope * x_vals
    else:
